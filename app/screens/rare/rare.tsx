            ...
            <View style={{flex: 1}}>
                <ImageBackground style={{width: '100%', height: '100%'}} source={require("../../assets/common_screen/common_back.jpg")}>
                    <View style={Common_styles.top_bar_container}>
                        <ImageBackground style={{width: '100%', height: '100%'}} source={require("../../assets/rare/rare_back.jpg")}>
                            <View style={Common_styles.top_bar_wrapper}>
                                ...
                                <View style={Common_styles.bar_text_wrapper}><Text style={Common_styles.top_bar_text}>РЕДКОЗЕМЕЛЬНЫЕ МЕТАЛЛЫ</Text></View>
...
const rareDB = new PouchDB('raredb');
...
class CommonList extends Component<Props, State> {
    ...
    async getData() {
        try {
            const data = await fetchList.fetchListData(AppUrls.rare_collection);
            const filter_data =await filterData<MetalInterface>(data, 'group');
            await saveDataToPounchDB(rareDB, data);
            ...
            rareDB.info().then(function(info) {
    ...
    _renderItem=({item})=>{
        return item.map((index, i)=>{
            return(
                <View key={index.slug} style={Common_styles.list_item_wrapper}>
